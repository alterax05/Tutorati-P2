...
public class Main {
    public static void main(String[] args) {
        ...
        a1.aggiungiCategoria(new Studente("UNIPD", true));
        archivio[0] = a1;

        Abbonato a2 = new Abbonato("Bianchi", "Giovanni", "3", 1995);
        a2.aggiungiCategoria(new Studente("UNITN", false));
        archivio[1] = a2;

        Abbonato a3 = new Abbonato("Ferrari", "Alberto", "7", 1993);
        a3.aggiungiCategoria(new Studente("UNITN", true));
        a3.aggiungiCategoria(new Atleta("atletica", true));
        archivio[2] = a3;

        Abbonato a4 = new Abbonato("Ferrari", "Vincenzo", "8", 1997);
        a4.aggiungiCategoria(new Studente("UNIVR", false));
        a4.aggiungiCategoria(new Atleta("atletica", false));
        archivio[3] = a4;
        ...
        a7.aggiungiCategoria(new Atleta("curling", true));
        archivio[6] = a7;

        Abbonato a8 = new Abbonato("Verdi", "Giacomo", "5", 1991);
        a8.aggiungiCategoria(new Atleta("nuoto", false));
        archivio[7] = a8;
        ...
        System.out.println("\n--- Filtro Studenti ---");
        for (int i = 0; i < archivio.length; i++) {
            ScontoStrategy[] cat = archivio[i].getCategorie();
            boolean isStudente = false;

            for (int j = 0; j < cat.length; j++) {
                if (cat[j] instanceof Studente) {
                    isStudente = true;
                }
            }
        ...
        for (int i = 0; i < archivio.length; i++) {
            ScontoStrategy[] cat = archivio[i].getCategorie();
            boolean isAtleta = false;

            for (int j = 0; j < cat.length; j++) {
                if (cat[j] instanceof Atleta) {
                    isAtleta = true;
                }
            }
